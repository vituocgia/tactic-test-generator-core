...
import pid.fsoft.tactictest.ui.crawljax.plugins.TacticTestOnUrlFirstLoadPlugin;
import pid.fsoft.tactictest.ui.util.TacticTestLogger;
import org.tomlj.*;
...
public class CrawljaxRunner {
    ...
    private static final Logger logger = TacticTestLogger.getLogger(CrawljaxRunner.class);
...
//        }
        ...
        BrowserConfiguration browserConfiguration = new BrowserConfiguration(
            browserType, 1, new TacticTestBrowserProvider(browserType)
        );
...
//        TomlParseResult clickableSpec = Toml.parse(Paths.get(clickablesSpecFile));
    ...
    private static void processPreCrawlActions(TomlParseResult precrawlActionsSpec,
        ...
        if (precrawlActionsSpec.contains("precrawl_action")) {
            ...
            builder.addPlugin(new TacticTestOnUrlFirstLoadPlugin(preCrawlActions));
        }
    }
...
//            createTestConfiguration(generateOptions.getString("add_state_diff_assertions"))));
    ...
    /**
     * Parses the configuration options specified in the given tactic-test toml config file,
